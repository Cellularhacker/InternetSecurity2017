...
int array_a1[] = {
    ...
    15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 0, 6, 13
};
...
void display_keys(key_set key_set) {
	...
	printf("Y: \n");
	for(count = 0; count < 8; count++) {
		printf("%02X : ", key_set.k[count]);
		binary_digits(key_set.k[count]);
		printf("\n");
	}

	printf("\nX: \n");
	for(count = 0; count < 4; count++) {
		printf("%02X : ", key_set.c[count]);
		binary_digits(key_set.c[count]);
		printf("\n");
	}

	printf("\nZ: \n");
	for(count = 0; count < 4; count++) {
		printf("%02X : ", key_set.d[count]);
		binary_digits(key_set.d[count]);
		printf("\n");
	}
...
void key_maker(unsigned char* main_key, key_set* key_sets) {
	...
	for(m = 0; m < 56; m++) {
		...
		key_sets[0].k[m/8] |= (shift_byte >> m%8);
	}

	for(m = 0; m < 3; m++) 
		key_sets[0].c[m] = key_sets[0].k[m];

	key_sets[0].c[3] = key_sets[0].k[3] & 0xF0;

	for(m = 0; m < 3; m++) {
		key_sets[0].d[m] = (key_sets[0].k[m + 3] & 0x0F) << 4;
		key_sets[0].d[m] |= (key_sets[0].k[m + 4] & 0xF0) >> 4;
	}
	...
	for(m = 1; m < 17; m++) {
		for(n = 0; n < 4; n++) {
			key_sets[m].c[n] = key_sets[m - 1].c[n];
			key_sets[m].d[n] = key_sets[m - 1].d[n];
		}
		shift_size = key_shift_sizes[m];
		...
		first_bit = shift_byte & key_sets[m].c[0];
		second_bit = shift_byte & key_sets[m].c[1];
		third_bit = shift_byte & key_sets[m].c[2];
		fourth_bit = shift_byte & key_sets[m].c[3];
		key_sets[m].c[0] <<= shift_size;
		key_sets[m].c[0] |= (second_bit >> (8 - shift_size));
		key_sets[m].c[1] <<= shift_size;
		key_sets[m].c[1] |= (third_bit >> (8 - shift_size));
		key_sets[m].c[2] <<= shift_size;
		key_sets[m].c[2] |= (fourth_bit >> (8 - shift_size));
		key_sets[m].c[3] <<= shift_size;
		key_sets[m].c[3] |= (first_bit >> (4 - shift_size));
		first_bit = shift_byte & key_sets[m].d[0];
		second_bit = shift_byte & key_sets[m].d[1];
		third_bit = shift_byte & key_sets[m].d[2];
		fourth_bit = shift_byte & key_sets[m].d[3];
		key_sets[m].d[0] <<= shift_size;
		key_sets[m].d[0] |= (second_bit >> (8 - shift_size));
		key_sets[m].d[1] <<= shift_size;
		key_sets[m].d[1] |= (third_bit >> (8 - shift_size));
		key_sets[m].d[2] <<= shift_size;
		key_sets[m].d[2] |= (fourth_bit >> (8 - shift_size));
		key_sets[m].d[3] <<= shift_size;
		key_sets[m].d[3] |= (first_bit >> (4 - shift_size));
		...
		for(n = 0; n < 48; n++) 
		{
			...
			if(shift_size <= 28) {
				shift_byte = 0x80 >> ((shift_size - 1)%8);
				shift_byte &= key_sets[m].c[(shift_size - 1)/8];
				shift_byte <<= ((shift_size - 1)%8);
			} else {
				shift_byte = 0x80 >> ((shift_size - 29)%8);
				shift_byte &= key_sets[m].d[(shift_size - 29)/8];
				shift_byte <<= ((shift_size - 29)%8);
			}
			key_sets[m].k[n/8] |= (shift_byte >> n%8);
		}
	}
}
...
void evaluate_message(unsigned char* message_piece, unsigned char* processed_piece, key_set* key_sets, int mode) 
{
	...
	for (temp = 1; temp <= 16; temp++) {
		...
		for(count = 0; count < 48; count++) {
			shift_size = message_piece[count];
			shift_byte = 0x80 >> ((shift_size - 1)%8);
		...
		for(count = 0; count < 6; count++) 
			er[count] ^= key_sets[key_position].k[count];
